...
%     solar_run_cost == sum(mpc.solar(:, 8) .* vars.Ps * 1000 * t);  % 光伏发电成本
%     wind_run_cost == sum(mpc.wind(:, 8) .* vars.Pw * 1000 * t);  % 风电发电成本
%     storage_run_cost == sum(mpc.storage(:, 7) .* (abs(vars.Pst_in) + abs(vars.Pst_out)) * 1000 * t + mpc.storage(:, 8));  % 储能电站运行成本
%     purchase_cost == 1000 * t * vars.Pg(ng, :)' .* mpc.price ;  % 购电成本
%     branch_cost == 1000 * t * sum(vars.l .* r .* mpc.price');  % 支路功率损耗成本（与电价相关）
% ];
...
% 1. 母线电压约束
C = [C;
    mpc.bus(:, 6).^2 .* Ob <= vars.v <= mpc.bus(:, 5).^2 .* Ob;
];

% 2. 发电机出力约束
C = [C;
    mpc.gen(:, 3) .* Og <= vars.Pg <= mpc.gen(:, 2) .* Og;   % 有功出力限制
    mpc.gen(:, 5) .* Og <= vars.Qg <= mpc.gen(:, 4) .* Og;   % 无功出力限制
    vars.Pg.^2 + vars.Qg.^2 <= mpc.gen(:, 6).^2 .* Og;  % 容量限制
];

% 3. 光伏出力约束
C = [C;
    mpc.solar(:, 3) .* Os <= vars.Ps <= mpc.solar(:, 2) .* Os;  % 有功出力限制
    0 <= vars.Ps <= mpc.solar(:, 5) .* Os .* pf_s;        % 容量限制
    vars.Ps <= mpc.solar(:, 5) .* mpc.solar_time;   % 日出力限制
];

% 4. 风电出力约束
C = [C;
    mpc.wind(:, 3) .* Ow <= vars.Pw <= mpc.wind(:, 2) .* Ow;  % 有功出力限制
    0 <= vars.Pw <= mpc.wind(:, 5) .* Ow .* pf_w;       % 容量限制
    vars.Pw <= mpc.wind(:, 5) .* mpc.wind_time;   % 日出力限制
];

% 5. 储能出力约束
C = [C;
    0 <= vars.Pst_in <= mpc.storage(:, 2) .* Ost;  % 有功出力限制
    0 <= vars.Pst_out <= mpc.storage(:, 3) .* Ost;  % 有功出力限制
    vars.state_in + vars.state_out <= 1;                 % 选择充放电状态
    0 <= vars.Pst_in <= 0.2 * mpc.storage(:, 5) .* vars.state_in .* Ost;    % 充电功率限制
    0 <= vars.Pst_out <= 0.2 * mpc.storage(:, 5) .* vars.state_out .* Ost;  % 放电功率限制
];
for i = 1:nt
    C = [C;
...
% 6. 节点功率平衡约束
p_j = mat_gbus * vars.Pg + mat_sbus * vars.Ps + mat_wbus * vars.Pw + mat_stbus * (vars.Pst_out - vars.Pst_in) - mpc.pd_time;
q_j = mat_gbus * vars.Qg + mat_sbus * (tan(acos(pf_s) .* Os .* vars.Ps)) + ...
    mat_wbus * (tan(acos(pf_w) .* Ow .* vars.Pw)) + mat_stbus * (tan(acos(pf_st) .* Ost .* (vars.Pst_out - vars.Pst_in))) - mpc.qd_time;
C = [C;
    p_j == mat_tbus * vars.P - mat_fbus * (vars.P - r .* ones(nl, nt) .* vars.l);
    q_j == mat_tbus * vars.Q - mat_fbus * (vars.Q - x .* ones(nl, nt) .* vars.l);
]

% 7. 支路功率流约束（基于分支流模型的二阶锥约束）
C = [C;
    vars.P.^2 + vars.Q.^2 <= vars.l .* (mat_fbus' * vars.v);  % 能写成cone([a, b; x, y])吗？感觉意义不大，计算差不了几秒。
    mat_tbus' * vars.v == mat_fbus' * vars.v - 2 * (r .* ones(nl, nt) .* vars.P + x .* ones(nl, nt) .* vars.Q) + (r.^2 + x.^2) .* ones(nl, nt) .* vars.l;
    0 <= vars.v;
    0 <= vars.l <= mpc.branch(:, 7).^2 .* ones(nl, nt);
];
