...
% 光伏发电数据
% [conn_bus pf S k a b eff]
case_mpc.pv = [
    4 0.9 0.2 0.0872 10800 0.05 0.7;
    19 0.9 0.2 0.0872 10800 0.05 0.7;
    23 0.9 0.2 0.0872 10800 0.05 0.7;
];

% 风力发电数据
% [conn_bus pf S k a b R C]
case_mpc.wind = [
    8 0.9 0.3 0.0872 11520 0.32 70 0.3757;
    14 0.9 0.3 0.0872 11520 0.32 95 0.3757;
    29 0.9 0.3 0.0872 11520 0.32 108 0.3757;
];

% 储能电站数据
% [conn_bus P_max P_min n S a b c]
% case_mpc.ess = [
%     6 0.2 0.1 0.9 0.23 0 0 0;
% ];
...
case_mpc.pd_time = cat(3, ...
    ...
    repmat([0 110 80 130 70 55 210 180 65 55 50 65 55 130 70 65 65 100 85 95 95 85 95 400 430 65 65 65 130 210 140 220 65]', 1, 24), ... % 场景2
    repmat([0 90 100 110 50 65 190 220 55 65 40 55 65 110 50 55 55 80 95 85 85 95 85 440 410 55 55 55 110 190 160 200 55]', 1, 24) ... % 场景3
);
...
case_mpc.qd_time = cat(3, ...
    ...
    repmat([0 65 35 85 35 18 110 90 25 18 35 40 30 90 15 25 25 45 35 45 45 35 55 210 210 30 30 25 80 610 60 110 45]', 1, 24), ... % 场景2
    repmat([0 55 45 75 25 22 90 110 15 22 25 30 40 70 5 15 15 35 45 35 35 45 45 190 190 20 20 15 60 590 80 90 35]', 1, 24) ... % 场景3
);
...
% 光伏日出力时段数据
case_mpc.pv_time = [
    [0 0 0 0 0 1 77 267 496 700 852 931 925 838 695 514 294 81 2 0 0 0 0 0]; ... % 场景1
    [0 0 0 0 0 4 90 324 548 735 887 947 924 816 640 405 174 47 1 0 0 0 0 0]; ... % 场景2
    [0 0 0 0 0 4 118 291 540 565 873 964 950 858 698 514 291 76 2 0 0 0 0 0]; ... % 场景3
];

% 风力日出力时段数据
case_mpc.wind_time = [
    [2 2 2 2 2 2 2 2 2 2 2 3 3 3 3 3 3 3 3 2 2 2 2 2];
    [2 2 2 2 2 2 2 2 2 3 3 3 3 3 3 2 2 3 5 6 5 4 4 4]; ... % 场景2
    [4 3 3 3 2 2 2 1 1 2 2 1 1 1 2 2 3 4 3 4 3 3 3 3];  ... % 场景3
];

case_mpc.prob = [0.4 0.3 0.3]; % 场景概率